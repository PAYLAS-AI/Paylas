...
class JobService {
  ...
  Future<List<Job>> getUnActiveJobs() async {
    final snapshot =
        await _db.collection('jobs').where('isActive', isEqualTo: false).get();
  ...
  Future<List<Job>> getActiveJobs() async {
    final snapshot =
        await _db.collection('jobs').where('isArchived', isEqualTo: false).where('isActive', isEqualTo: true).get();
  ...
  Future<Job?> showJob(String jobId) async {
    final docs =
        await _db.collection('jobs').where('id', isEqualTo: jobId).get();
  ...
  Future<void> archiveJob(String jobId) async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      ...
      await _db.collection('jobs').doc(docId).update({'isArchived': true});
    } else {
  ...
  Future<void> deleteDocByInnerId(String targetId) async {
    ...
    final querySnapshot =
        await collection.where('id', isEqualTo: targetId).get();
